...
export const RULES: Record<string, RuleDoc> = {
  ...
  "add-domain-not-null": {
    ...
    title: "ALTER DOMAIN ADD/SET NOT NULL",
    severity: "error",
    category: "Constraints",
    summary:
      "Making a domain `NOT NULL` scans and write-locks every dependent table, with no `NOT VALID` escape.",
    whyUnsafe:
    ...
    example: { unsafe: "ALTER DOMAIN us_postal SET NOT NULL;" },
  ...
  "prefer-jsonb": {
    ...
    related: ["forbidden-column-type", "prefer-identity"],
  },
  "prefer-identity": {
    id: "prefer-identity",
    ...
    category: "Schema design",
    summary:
      "`serial` creates an implicitly-owned sequence with sharp edges; identity columns are cleaner and SQL-standard.",
    whyUnsafe:
    ...
    example: {
      unsafe: "CREATE TABLE t (id bigserial PRIMARY KEY);",
      safe: "CREATE TABLE t (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY);",
    },
    related: ["add-column-serial", "add-column-identity", "prefer-bigint-primary-key"],
  },
  "prefer-bigint-primary-key": {
    ...
    severity: "warning",
    category: "Schema design",
    summary: "A small-integer primary key overflows its id space; use `bigint` or `bigint GENERATED BY DEFAULT AS IDENTITY`.",
    whyUnsafe:
    ...
    safeRewrite:
      "Use `bigint`, or `bigint GENERATED BY DEFAULT AS IDENTITY`. Migrating a live `int` primary key to `bigint` later is a major, painful operation — start with `bigint`.",
    example: {
      unsafe: "CREATE TABLE events (id serial PRIMARY KEY);",
      safe: "CREATE TABLE events (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY);",
    },
  ...
  "require-primary-key": {
    ...
    example: {
      ...
      safe: "CREATE TABLE events (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, payload jsonb);",
    },
