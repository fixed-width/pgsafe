...
#[test]
fn add_domain_not_null_fires() {
    assert!(fires("ALTER DOMAIN d ADD NOT NULL", "add-domain-not-null"));
    assert!(fires(
...
fn add_column_not_null_no_default() {
    ...
    // A column-level PRIMARY KEY implies NOT NULL with no default — same failure.
    assert!(fires(
...
#[test]
fn prefer_identity_fires() {
    assert!(fires("CREATE TABLE t (id serial)", "prefer-identity"));
    assert!(fires("CREATE TABLE t (id bigserial)", "prefer-identity"));
    assert!(fires(
        "ALTER TABLE t ADD COLUMN id serial",
        "prefer-identity"
    ));
...
#[test]
fn prefer_identity_silent() {
    assert!(!fires("CREATE TABLE t (id int)", "prefer-identity"));
    assert!(!fires(
