...
- `pgsafe lsp`: a stdio Language Server providing live editor diagnostics and
...
- `pgsafe --help` no longer prints the parser's internal Rust doc comment as its long
  description; it shows just the one-line command summary.
- `prefer-bigint-primary-key` no longer gives advice that contradicts `prefer-identity`. Its guidance recommended `bigserial` — which `prefer-identity` then flags — and mismatched the identity flavor (`GENERATED ALWAYS` vs `BY DEFAULT`). It now recommends `bigint` or `bigint GENERATED BY DEFAULT AS IDENTITY`, and its autofix rewrites a `serial` primary key straight to a wide identity column (clearing both rules in one step) instead of to `bigserial`. Non-serial integer keys still widen to `bigint`.
